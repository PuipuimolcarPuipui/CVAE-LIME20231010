...
class LimeTabularExplainer(object):
    ...
    def __init__(self,
        ...
        # オートエンコーダの訓練
        if auto_encoder_setting['auto_encoder_training'] == True:
            from limes.cvae_lime.auto_encoders.AE import AE_training
            AE_training(X_train = training_data,
        ...
        self.base = lime_base.LimeBase(kernel_fn, verbose, random_state=self.random_state, auto_encoder_setting=self.auto_encoder_setting)
    ...
    def explain_instance(self,
        ...
        # インスタンス番号を辞書に追加
        self.auto_encoder_setting['instance_no'] = instance_no
        # self.auto_encoder_setting['X_test'] = instance_no
        ...
        # サンプリングする場合はLIMEのサンプル生成をスキップする
        if self.auto_encoder_setting['auto_encoder_sampling'] == False:
            data, inverse = self.__data_inverse(data_row, num_samples)
        else:
            data = np.tile(data_row, (num_samples, 1))
            inverse = np.tile(data_row, (num_samples, 1))
        ...
        for label in labels:
            (ret_exp.intercept[label],
             ...
             ret_exp.score, ret_exp.local_pred, ret_exp.Active_latent_dim, ret_exp.significant_coeffs) = self.base.explain_instance_with_data(
                    scaled_data,
                    yss,
                    distances,
                    label,
